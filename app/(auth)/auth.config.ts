...
export const authConfig = {
  ...
  callbacks: {
    authorized({ auth, request: { nextUrl } }) {
      ...
      const isOnPublicApi = nextUrl.pathname.startsWith('/api/chat') || nextUrl.pathname.startsWith('/api/files'); // Or other public API paths
      ...
      if (isOnAdminArea || isOnAdminApi) {
        if (isOnAdminLogin) {
          if (isLoggedIn && auth?.user?.isAdmin) {
            ...
            return Response.redirect(new URL('/admin/dashboard/users', nextUrl as unknown as URL));
          }
        ...
        if (!isLoggedIn) {
          return Response.redirect(new URL('/admin/login', nextUrl as unknown as URL));
        }
